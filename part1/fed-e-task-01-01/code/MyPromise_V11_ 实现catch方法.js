...
-----------------------------------------------------------------------------------------------------
V8: Promise.all方法用来解决异步并发问题；	允许按照异步代码调用的顺序，来得到异步代码执行的结果。
	语法：
		接收数组作为参数，数组中可以使用普通值和promise对象；
		数组中值得顺序 就是最后得到得 结果得顺序。
	返回值也是promise对象;
	所以后面也是可以链式调用then方法的
	特点： all 方法中所有的方法都是成功的，得到的结果的状态才是成功的，否则就是失败的。

	Promise.all方法的实现。 
		因为调用的方式是：Promise.all 所以是个静态方法 ，用static 声明
		接收的形参是个 array
		声明存放结果数组的变量

		返回的是promise 对象
			在 promise 对象的执行器函数中遍历参数数组

		循环数组 , 判断数组中的值是普通值还是promise对象
			如果是普通值，就直接将该值放到结果数组中；
			如果是promise对象，就先去执行promise对象再把结果放到数组中

		注意for循环时不会等待里面的异步任务，所以 如果在for循环下面直接调用resove 会出现没能获取到promise返回值的bug：[ 'a', 'b', <1empty item>, 'p2', 'c', 'd' ]
			因此可以通过计数器来判断 是不是数组中所有的值都执行结束，结束之后再调用 resolve 可以确保获取到所有的结果:[ 'a', 'b', 'p1', 'p2', 'c', 'd' ]

-----------------------------------------------------------------------------------------------------
V9: Promise.resolve方法用来将给定的值转换成Promise对象
	判断参数是不是promise对象：
		参数为promise对象时，原封不动的return 出去。
		参数为普通值时，创建一个promise对象，将给定的值包裹在promise对象中，然后再返回出去。

	获取返回值： 同样时调用 then方法

-----------------------------------------------------------------------------------------------------
V10: finally 方法(非静态，定义在原型对象中)。	
	特点：
		无论当前的promise的状态是成功/失败，都会执行。
		支持链式调用

	参数： 接收一个回调函数callback 
	通过then 方法确定 当前promise的状态，
	分别在 成功和失败的回调中 调用 callback

	返回promise对象，才能继续支持then方法

-----------------------------------------------------------------------------------------------------
V11: catch 方法(定义在原型对象中)。	 
	catch 方法后面 同样可以链式调用其他方法
			需要return 一个 promise对象。

	在catch 方法的内部 调用 then 方法，注册一个failCallback 回调。
		在成功回调的地方 传入 undefined
		在失败回调的地方 传入 failCallback
...
class MyPromise {
	...
	// v10 定义finally 方法
		...
		// 返回promise对象，用于支持链式调用
		// 调用then 方法后，无论哪种状态都继续调用callback
		return this.then(
			value => {
				callback()
				return value; // 将结果传给下一个then方法
			},
			reason => {
				callback()
				throw reason;  // 将结果传给下一个then 方法
			}
		)
	}
	...
	static all ( array ) {
		let results = []
		let index = 0; // 计数，解决for循环瞬间执行完，但是里面的异步方法还没有执行的bug

		return new MyPromise( ( resolve, reject ) => {
			// 封装方法,将值push进结果数组中
			function addData2Result (key,value) {
				results[ key ] = value;
				index++;
			...
			for ( let i = 0; i < array.length; i++ ) {
				...
				if ( current instanceof MyPromise ) {
					// promise对象
					current.then( value => 	addData2Result(i, value) , reason => reject(reason) )
				} else {
					addData2Result(i,array[i]) // 普通值
				}
			}
